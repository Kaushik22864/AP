        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME))) {
            ...
            for (int i = 0; i < tourists.size(); i++) {
                ...
                writer.write(String.format(
                        "  {\"name\":\"%s\", \"nationality\":\"%s\", \"contact\":\"%s\", \"emergencyContact\":\"%s\"}%s\n",
                        escape(t.getName()), escape(t.getNationality()),
                        escape(t.getContact()), escape(t.getEmergencyContact()),
                        (i < tourists.size() - 1 ? "," : "")
                ));
            }
            writer.write("]");
    ...
    public static List<Tourist> loadTourists() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            ...
            for (String entry : entries) {
                ...
                String name = "", nationality = "", contact = "", emergencyContact = "";
                ...
                try {
                    tourists.add(new Tourist(name, nationality, contact, emergencyContact));
