        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME))) {
            ...
            for (int i = 0; i < bookings.size(); i++) {
                ...
                writer.write(String.format(
                        "  {\"tourist\":\"%s\", \"guide\":\"%s\", \"attraction\":\"%s\", \"date\":\"%s\", \"status\":\"%s\", \"cost\":%.2f}%s\n",
    ...
    public static List<Booking> loadBookings() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            ...
            for (String entry : entries) {
                ...
                try {
                    Tourist tourist = new Tourist(touristName, "Nepal", "N/A", "N/A");
                    Guide guide = new Guide(guideName, List.of("English"), 1, "N/A");
                    Attraction attraction = new Attraction(attractionName, "Cultural", "Kathmandu", "Description");

                    bookings.add(new Booking(tourist, guide, attraction, date, status, cost));
                } catch (Exception e) {
